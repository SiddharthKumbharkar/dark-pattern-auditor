    ...
    <div className="mx-auto max-w-2xl px-6 py-14">
      ...
      <div className="mt-6 rounded-2xl border border-brand-border bg-brand-card p-6">
        ...
        {status === "cancelled" && (
          <div className="mt-3">
            ...
            <Link to="/trial" className="mt-4 inline-block text-sm font-medium text-brand-accent-dark underline">
              Start a new trial
            </Link>
          </div>
        )}
      ...
      {darkPatternsEnabled && (status === "trial_active" || status === "active") && (
        <div className="mt-10 text-center">
          <Link to="/account/subscription" className="text-xs text-brand-muted/70 underline">
            Manage membership
          </Link>
        </div>
      )}
